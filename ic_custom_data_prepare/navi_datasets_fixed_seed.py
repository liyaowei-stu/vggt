...
class MVImageDataset(Dataset):
    def __init__(self, meta_path, data_dir, min_recon_num=8, max_recon_num=8):
        """
    ...
    def reorganize_data(self):
        ...
        # 确保路径列表的顺序是确定的
        self.multi_view_img_paths = sorted(list(set(multi_view_img_paths)))
        self.video_paths = sorted(list(set(video_paths)))
        self.multi_view_paths = sorted(self.multi_view_img_paths + self.video_paths)
        self.wild_set_paths = sorted(list(set(wild_set_paths)))
    ...
    def set_recon_num(self, recon_num=None):
        ...
        if recon_num is None:
            # 使用固定种子的随机生成器
            self.current_recon_num = random.Random(SEED).randint(self.min_recon_num, self.max_recon_num)
        else:
            self.current_recon_num = recon_num
        return self.current_recon_num

    def __getitem__(self, idx):
        # 为每个样本创建确定性随机生成器，使用idx作为种子的一部分
        # 这样不同样本有不同但可重现的随机结果
        sample_random = random.Random(SEED + idx)
        ...
        while True:
            try:
                ...
                if self.current_recon_num is not None:
                    ...
                    recon_num = sample_random.randint(self.min_recon_num, self.max_recon_num)
                ...
                if len(gt_images) < recon_num:
                    ...
                    sample_recon_idx = available_indices + [sample_random.choice(available_indices) for _ in range(recon_num - len(available_indices))]
                else:
                    # Normal case: randomly sample recon_num indices
                    # 先排序确保顺序一致性，再采样
                    indices = list(range(len(gt_images)))
                    sample_random.shuffle(indices)
                    sample_recon_idx = sorted(indices[:recon_num])
                ...
                ## render info 
                sample_render_idx = sample_random.randint(0, len(predictions["images"]) - 1)
...
class BatchSampler(torch.utils.data.Sampler):
    ...
    def __iter__(self):
        ...
        # 使用固定种子设置recon_num
        fixed_recon_num = self.random_generator.randint(self.dataset.min_recon_num, self.dataset.max_recon_num)
...
def loader(train_batch_size, num_workers, shuffle=False, **args):
    ...
    return DataLoader(dataset, batch_sampler=batch_sampler, num_workers=num_workers, collate_fn=collate_fn, worker_init_fn=lambda worker_id: np.random.seed(SEED + worker_id))
